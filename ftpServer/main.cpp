...
#define errmsg(msg) do{cout<< msg << endl;exit(1);}while(0)  //输出错误信息并退出程序
/**************basic include***************/
#include <string.h>
#include <string>
using std::string;
...
#define SPORT 21
#define BUFS 1024 //缓冲区大小

#define XThreadPoolGet XThreadPool::Get()  //通过宏来简写一个函数调用
...
int main() {
	...
	if (signal(SIGPIPE, SIG_IGN) == SIG_ERR)//忽略SIGPIPE 避免因为向一个已经关闭的 socket 写数据而导致程序终止
	...
	evconnlistener *ev = evconnlistener_new_bind(
		...
		base,											//回调函数获取的参数 arg
		LEV_OPT_REUSEABLE|LEV_OPT_CLOSE_ON_FREE,		//地址重用(即使之前程序退出后，占用的端口仍然可以被其他程序使用) evconnlistener关闭同时关闭socket
		10,												//连接队列大小,允许的未处理连接的最大数量,对应listen函数
		(sockaddr*)&sin,								//绑定的地址和端口
		sizeof(sin));
	
	cout << "ev:" << ev << endl;
	cout << "err:" << errno << endl;
	if (base) {//进入 libevent 的事件循环，开始监听和处理事件
